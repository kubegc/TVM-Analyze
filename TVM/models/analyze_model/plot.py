...
json_path = os.path.join(config.dataset_path,"datasets_model/dataset.json") 
datas = {}
picture_index = 1
...
device={}
with open(config.device_json_path,'r') as f:
    device = json.load(f)
                ...
                for shape in datas[device_name][op_name][device_id][shapes_dimensionality].keys():
                    ...
                    with open(os.path.join(config.prefix_dataset_fold_path, value["file_path"])) as f:
                        ...
                        while line is not None and len(line)>0 :
                            ...
                            line=f.readline()
                    img = plt.subplot(math.ceil(picture_count),1 , img_index)
                    plt.plot(*data)
                    # plt.legend() # 显示图例
