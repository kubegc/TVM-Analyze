...
def func(x, x0,x1,y0,y1,y2):
    return np.piecewise(x, [x <= x0, np.logical_and(x0<x, x<= x1),x>x1],[lambda x:y0,lambda x:y1,lambda x:y2])
        ...
        for device_id in datas[device_name][op_name].keys():
            ...
            for shapes_dimensionality in datas[device_name][op_name][device_id].keys():
                ...
                for shape in datas[device_name][op_name][device_id][shapes_dimensionality].keys():
                    ...
                    plt.ylabel('run-time')
                    img.set_title(shape)
                plt.savefig(os.path.join(fold_path,hardware_name+"--k_curve.png"))